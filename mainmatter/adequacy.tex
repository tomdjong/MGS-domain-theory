...
equal.
%
Even if we did allow for reductions in the bodies of the
\(\lambda\)-abstractions, we could easily get another counterexample such as the
pair of programs
\(\lambdadot{\var x : \pcfnat}{\lambdadot{\var y :
    \pcfnat}{\pcffont{add}\,x\,y}}\) and
\(\lambdadot{\var x : \pcfnat}{\lambdadot{\var y :
    \pcfnat}{\pcffont{add}\,y\,x}}\) where \(\pcffont{add}\) is a program for
addition.

...
\begin{example}
  ...
  %
  In fact, it will serve as the interpretation of the PCF programme
...
\begin{exercise}\label{exer:approximating-successor}
  ...
  \begin{align*}
    s_k \colon \Nat_\bot &\to \Nat_\bot \\
    \bot &\mapsto \bot \\
    n &\mapsto \begin{cases} n+1 &\text{if \(n \leq k\)}, \\ \bot &\text{else}. \end{cases}
  \end{align*}
  \begin{enumerate}[(i)]
  \item Show that each \(s_k\) is \(\omega\)-continuous.
  \item Show that \(\set{s_k \mid k \in \Nat}\) is an \(\omega\)-chain in
    \(\Nat_\bot^{\Nat_\bot}\) with least upper bound
    \begin{align*}
...
fixed point (\cref{least-fixed-point}) is continuous.
...
The \(\omega\)-continuous function
\(\mu \colon \densem{\sigma}^{\densem\sigma} \to \densem\sigma\) will be used to

...
\begin{theorem}[Knaster--Tarski fixed point theorem]\label{least-fixed-point}
  ...
  fixed point \(x_0\). In fact,
  \begin{enumerate}[(i)]
  \item \(x_0 = f(x_0)\), and
  \item for every \(y \in X\) with \(f(y) \below y\), we have \(x_0 \below y\).
  \end{enumerate}
  Moreover, we can explicitly calculate \(x_0\) as the least upper bound of the
  \(\omega\)-chain \(\set{\bot,f(\bot),f(f(\bot)),\dots}\).
\end{theorem}

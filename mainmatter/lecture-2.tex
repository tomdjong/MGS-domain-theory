...
\begin{lemma}
  ...
  \end{align*}
  are morphisms of \(\omega\)-cpos.
\end{lemma}

\begin{exercise}\label{exer:product-pairwise}
  Show that a function \(f \colon A \times B \to C\) between \(\omega\)-cpos is
  a morphism of \(\omega\)-cpos if and only if the functions
  \(f(x,-) \colon B \to C\) and \(f(-,y) \colon A \to C\) are morphisms of
  \(\omega\)-cpos for every \(x \in A\) and \(y \in B\).
\end{exercise}
